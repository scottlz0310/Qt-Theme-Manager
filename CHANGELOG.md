...
### Added
- モダンな開発環境への移行（uv、ruff、mypy導入）
- ローカルCI/CD検証ツールの追加
- pre-commitフックによる自動コード品質チェック

### Fixed
- Ubuntu環境でのQt GUI依存関係の不足を解決
- Windows環境でのテスト失敗を修正（改行コード処理、ファイルロック問題）
- macOS環境でのテスト失敗を修正（ケース非依存ファイルシステム、OSError処理）
- パフォーマンステストの安定性向上（複数回実行による平均化）
- releaseワークフローの権限設定とQt依存関係の追加
- workflow_dispatchでのバージョン自動更新機能
- Bandit警告の適切な抑制
...
- requirements.txt生成コマンドの更新

### Tests
- クロスプラットフォームテストの安定化（Ubuntu/Windows/macOS）
- テストカバレッジの維持（95%以上）
