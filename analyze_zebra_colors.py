...
def analyze_zebra_colors():
    ...
    for theme_name, theme_config in config["available_themes"].items():
        ...
        if bg_color == zebra_color:
            print(
                f"\n📋 {theme_name.upper()} "
                f"({theme_config.get('display_name', theme_name)})"
            )
            print("   ⚠️  ゼブラ色が設定されていません（背景色と同じ）")
            continue
        ...
        print(
            f"   背景色: {bg_color} RGB{color_diff['rgb1']} "
            f"HSL({color_diff['hsl1'][0]:.0f}°, "
            f"{color_diff['hsl1'][1]:.1f}%, {color_diff['hsl1'][2]:.1f}%)"
        )
        print(
            f"   ゼブラ色: {zebra_color} RGB{color_diff['rgb2']} "
            f"HSL({color_diff['hsl2'][0]:.0f}°, "
            f"{color_diff['hsl2'][1]:.1f}%, {color_diff['hsl2'][2]:.1f}%)"
        )
        ...
        if contrast_ratio < 1.2:
            print("   🟢 非常に控えめ（推奨: 視覚疲労軽減）")
        elif contrast_ratio < 2.0:
            print("   🟡 控えめ（バランス良好）")
        elif contrast_ratio < 3.0:
            print("   🟠 やや目立つ（従来的）")
        else:
            print("   🔴 高コントラスト（アクセシビリティ特化）")
    ...
    if results:
        ...
        if avg_contrast < 1.3:
            ...
            print(
                "  💭 より判別しやすくしたい場合は、明度差を2-4%増やすことを検討"
            )
        elif avg_contrast < 2.0:
            print("  ✅ 現在の設定はバランスが良好です")
        else:
            print(
                "  ⚠️  コントラストがやや高めです。アクセシビリティ目的でない限り、"
            )
