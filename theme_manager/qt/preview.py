...
class ThemePreviewWindow(QMainWindow if qt_available else object):
    ...
    def __init__(self, config_path: Optional[Union[str, Path]] = None):
        ...
        if not qt_available:
            raise RuntimeError(
                "Qt framework not available. Install PyQt5 or PySide6."
            )
    ...
    def _create_theme_buttons(self, main_layout):
        ...
        for theme_name, theme_config in themes.items():
            ...
            btn.clicked.connect(
                lambda checked, name=theme_name: self._on_theme_button_clicked(
                    name
                )
            )
...
def show_preview(
    ...
    if not qt_available:
        print(
            "Qt framework not available. Install PyQt5 or PySide6 to use preview."
        )
        return None
...
def launch_preview():
    ...
    parser = argparse.ArgumentParser(
        description="Qt Theme Manager - プレビューツール"
    )
    parser.add_argument(
        "--config",
        "-c",
    ...
    parser.add_argument(
        "--theme", "-t", type=str, help="起動時に適用するテーマ名"
    )
