...
class ZebraPatternGenerator:
    ...
    @staticmethod
    def adjust_brightness_hsl(
        hex_color: str, lightness_change_percent: float
    ) -> str:
        """HSL色空間で明度を調整（より精密）"""
        r, g, b = ZebraPatternGenerator.hex_to_rgb(hex_color)
    ...
    def adjust_brightness_hsv(hex_color: str, brightness_factor: float) -> str:
        ...
        return ZebraPatternGenerator.rgb_to_hex(
            int(r * 255), int(g * 255), int(b * 255)
        )

    @staticmethod
    def generate_zebra_color(
        base_color: str, contrast_target: float = 1.2, method: str = "auto"
    ) -> str:
        """
        ベース色からゼブラパターン色を生成
        ...
        for adjustment in adjustment_range:
            ...
            if method in ["auto", "darker", "lighter", "hsl"]:
                test_color = ZebraPatternGenerator.adjust_brightness_hsl(
                    base_color, adjustment
                )
            else:  # hsv
                test_color = ZebraPatternGenerator.adjust_brightness_hsv(
                    base_color, adjustment / 100.0
                )

            contrast = ZebraPatternGenerator.get_contrast_ratio(
                base_color, test_color
            )

            # 目標に最も近いコントラスト比を選択
            if abs(contrast - contrast_target) < abs(
                best_contrast - contrast_target
            ):
    ...
    @staticmethod
    def generate_accessibility_compliant_zebra(
        base_color: str, accessibility_level: str = "subtle"
    ) -> dict:
        """
        ...
        for method in methods:
            ...
            actual_contrast = ZebraPatternGenerator.get_contrast_ratio(
                base_color, zebra_color
            )
        ...
        best_method = min(
            zebra_colors.keys(),
            key=lambda m: abs(
                zebra_colors[m]["contrast_ratio"] - target_contrast
            ),
        )
        ...
        return {
            ...
            "luminance_difference": zebra_colors[best_method][
                "luminance_diff"
            ],
...
class ZebraPatternEditor(QWidget if qt_available else object):
    ...
    def setup_contrast_controls(self, parent_layout):
        ...
        self.contrast_spinbox.valueChanged.connect(
            self.on_contrast_spinbox_changed
        )
    ...
    def setup_accessibility_controls(self, parent_layout):
        ...
        for level, label, tooltip in levels:
            ...
            btn.clicked.connect(
                lambda checked, l=level: self.set_accessibility_level(l)
            )
    ...
    def update_preview(self):
        """プレビューを更新"""
        # カラープレビューを更新
        self.base_preview.setStyleSheet(
            f"background-color: {self.base_color};"
        )
        self.zebra_preview.setStyleSheet(
            f"background-color: {self.zebra_color};"
        )

        # 情報を更新
        contrast_ratio = ZebraPatternGenerator.get_contrast_ratio(
            self.base_color, self.zebra_color
        )
...
if __name__ == "__main__":
    ...
    if qt_available:
        ...
        print(
            "Qt framework not available. Please install PyQt5/PyQt6/PySide6."
        )
